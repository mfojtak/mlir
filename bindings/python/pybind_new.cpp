...
template class Mutable<int>;
using MInt = Mutable<int>;

template class Mutable<float>;
using MFloat = Mutable<float>;
...
struct PythonValueHandle {
  ...
  operator const ValueHandle& () const { return value; }
...
struct PythonMLIRModule {
  ...
  void invoke(const std::string &name, py::list& args) {
    std::vector<void *> args_sv;
    for (py::handle arg : args) {
      py::buffer buf = arg.cast<py::buffer>();
      py::buffer_info info = buf.request();
      args_sv.push_back(info.ptr);
    }
    
    auto invocationResult =
      engine->invoke(name, MutableArrayRef<void *>(args_sv));
    assert(!invocationResult && "call failed");
  }
  void test(py::list& args) {
    for(py::handle arg : args) {
      py::buffer buf = arg.cast<py::buffer>();
      py::buffer_info info = buf.request();
      void* data = info.ptr;
...
PYBIND11_MODULE(mlir_bindings, m) {
  m.doc() =
  ...
  py::class_<MInt>(m, "Int", py::buffer_protocol())
        .def_buffer(MInt::info)
        .def(py::init<int>())
        .def(py::init<>())
        .def_property("value", &MInt::value, &MInt::set_value)
        .def("__int__", &MInt::value)
        .def("__index__", &MInt::value);
  
  py::class_<MFloat>(m, "Float", py::buffer_protocol())
        .def_buffer(MFloat::info)
        .def(py::init<float>())
        .def(py::init<>())
        .def_property("value", &MFloat::value, &MFloat::set_value)
        .def("__float__", &MFloat::value);
  ...
  py::class_<PythonMLIRModule>(
      ...
      .def("invoke", &PythonMLIRModule::invoke,
           "Invokes the function using "
           "ExecutionEngine backed by the ORC JIT.")
      .def("test", &PythonMLIRModule::test,
           "test")
